...
def check_or_create_app_settings_json():
    ...
    # If neither exists, create a default appsettings.json in the root
        ...
        chroma_data_dir = os.path.expanduser('~/.chromaflow/data')
        models_dir = os.path.expanduser('~/.chromaflow/models')
        ...
        default_config = {
            ...
            "proxy_endpoint": "",
            "chromaDB_path": os.path.join(chroma_data_dir, 'TestDB'),
...
def load_settings_from_json(file_path):
    ...
    # Get values and expand paths
    address_flask = settings.get("flask_server_endpoint", address_flask)
    CHROMA_DATA_PATH = expand_path(settings.get("chromaDB_path", CHROMA_DATA_PATH))
    EMBEDMODEL = settings.get("embedding_model", EMBEDMODEL)
    EMBEDMODEL_LOCAL_PATH = expand_path(settings.get("embedding_model_path", EMBEDMODEL_LOCAL_PATH))
...
try:
    ...
    if os.path.exists(EMBEDMODEL_LOCAL_PATH):
        ...
        os.environ['HF_HOME'] = os.path.expanduser('~/.huggingface/cache')
        os.makedirs(os.environ['HF_HOME'], exist_ok=True)
        
        # Load the model with explicit cache directory
        pyEmbedFunction = embedding_functions.SentenceTransformerEmbeddingFunction(
            model_name=EMBEDMODEL_LOCAL_PATH,
            cache_folder=os.path.expanduser('~/.huggingface/cache')
        )
        print(f"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        ...
        # Download the model with explicit cache directory
    ...
    print(f"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    user_input = input("\n\nWould you like to continue without transformers model (Embeddings won't work though)? (y/n): ").strip().lower()
    
    if user_input == 'y':
        print("Continuing as a viewer. Embeddings won't work.")
        # Proceed with the app functionality as a viewer (without embeddings)
    else:
        print("Exiting the application gracefully...")
        exit(1)  # Exit if the user doesn't want to continue
...
def open_browser():
    time.sleep(5)
    # Ensure the URL is properly formatted for webbrowser.open
    url = address_flask if address_flask.startswith('http') else f'http://{address_flask}'
    webbrowser.open(url)
...
def write_default_config_route():
    ...
    default_config = {
        "flask_server_endpoint": "http://127.0.0.1:5000",
        "proxy_endpoint": "",
        "chromaDB_path": os.path.expanduser("~/.chromaflow/data/TestDB"),
        "embedding_model_selected_preset": "all-MiniLM-L6-v2",
        "embedding_model": "all-MiniLM-L6-v2",
        "embedding_context_window": 512,
        "embedding_model_dimension": 384,
        "embedding_model_path": os.path.expanduser("~/.chromaflow/models/all-MiniLM-L6-v2"),
        "language": "en-GB",
        "CPURAMIntervalEnabled": False,
        "CPURAMInterval": 2000, 
        "autoGenCollectionName": "testCollection",
...
def get_collections():
    ...
    try:
        ...
        print(f"   FROM: /get-collections   Checking ChromaDB Version...")
        chromadb_version = get_chromadb_app_version()
        if chromadb_version is None:
            print(f"   FROM: /get-collections   ERROR: Could not determine ChromaDB version")
            return jsonify({'error': 'Could not determine ChromaDB version'}), 500

        print(f"   FROM: /get-collections   Confirmed! - ChromaDB version: {chromadb_version}")

        # Remove 'Version: ' prefix and convert the version string to a tuple of integers
        version_str = chromadb_version.replace('Version: ', '').strip()
        version_tuple = tuple(map(int, version_str.split('.')))

        # Define the threshold version as a tuple (0, 6, 0)
        threshold_version = (0, 6, 0)

        if version_tuple >= threshold_version:
            print(f"   FROM: /get-collections   Using Chroma v0.6.0 or higher")
            print(f"   FROM: /get-collections   Fetching all collections using new method...")
            collections = persistentChromaClient.list_collections()
            collections_list = [{"name": collection} for collection in collections]
        else:
            print(f"   FROM: /get-collections   Using Chroma v0.5.20 or lower")
            print(f"   FROM: /get-collections   Fetching all collections using legacy method...")
            collections = persistentChromaClient.list_collections()
            print(f"   FROM: /get-collections   Collections Fetched...")

            # Convert each collection object into a serializable dictionary
...
def get_collection_overview():
    ...
    try:
        ...
        # Check ChromaDB version
        print(f"   FROM: /get-collection-overview   Checking ChromaDB Version...")
        chromadb_version = get_chromadb_app_version()
        if chromadb_version is None:
            print(f"   FROM: /get-collection-overview   ERROR: Could not determine ChromaDB version")
            return jsonify({'error': 'Could not determine ChromaDB version'}), 500

        print(f"   FROM: /get-collection-overview   Confirmed! - ChromaDB version: {chromadb_version}")
        ...
        if version_tuple >= threshold_version:
            # Chroma v0.6.0 or higher: We need to loop through collection names and fetch each collection's details
            ...
            # Chroma < v0.6.0: Use the original method where collection has attributes like .name, .id, .count()
...
def get_collection_details():
    ...
    try:
        ...
        # If configuration_json exists, extract hnsw_configuration and add its attributes
        if 'hnsw_configuration' in coll.configuration_json:
            hnsw_config = coll.configuration_json['hnsw_configuration']

            collection_details.update({
                'config_name': hnsw_config.get('name', 'N/A'),
                'config_space': hnsw_config.get('space', 'N/A'),
                'config_ef_construction': hnsw_config.get('ef_construction', 'N/A'),
                'config_ef_search': hnsw_config.get('ef_search', 'N/A'),
                'config_num_threads': hnsw_config.get('num_threads', 'N/A'),
                'config_m': hnsw_config.get('m', 'N/A'),
                'config_resize_factor': hnsw_config.get('resize_factor', 'N/A'),
                'config_batch_size': hnsw_config.get('batch_size', 'N/A'),
                'config_sync_threshold': hnsw_config.get('sync_threshold', 'N/A'),
            })
...
def delete_all_collections():
    ...
    try:
        ...
        print(f"   FROM: /api/delete-all-collections-v2   Checking ChromaDB Version...")
        chromadb_version = get_chromadb_app_version()
        if chromadb_version is None:
            print(f"   FROM: /api/delete-all-collections-v2   ERROR: Could not determine ChromaDB version")
            return jsonify({'error': 'Could not determine ChromaDB version'}), 500

        print(f"   FROM: /api/delete-all-collections-v2   Confirmed! - ChromaDB version: {chromadb_version}")

        # Remove 'Version: ' prefix and convert version string to tuple of integers
        version_str = chromadb_version.replace('Version: ', '').strip()
        version_tuple = tuple(map(int, version_str.split('.')))
        ...
        # Check version and apply corresponding deletion logic
        if version_tuple >= threshold_version:
            # New method for ChromaDB version >= 0.6.0
            for collection in collections:
                try:
            ...
            # Old method for ChromaDB version < 0.6.0
            for collection in collections:
                try:
...
def count_all_documents():
    ...
    try:
        # Check ChromaDB version
        print(f"   FROM: /count-all-documents   Checking ChromaDB Version...")
        chromadb_version = get_chromadb_app_version()
        if chromadb_version is None:
            print(f"   FROM: /count-all-documents   ERROR: Could not determine ChromaDB version")
            return jsonify({'error': 'Could not determine ChromaDB version'}), 500

        print(f"   FROM: /count-all-documents   Confirmed! - ChromaDB version: {chromadb_version}")
        ...
        # Check the ChromaDB version and apply the corresponding logic
        if version_tuple < threshold_version:
            print(f"   FROM: /count-all-documents   Using logic for ChromaDB < 0.6.0")
            # Loop through all collections to sum their document counts (for < 0.6.0)
            for collection in allCollections:
                try:
                    collection_name = collection.name
                    collection_id = collection.id
            ...
            for collection in allCollections:
                try:
                    # Attempt to get the collection
                    tempCollection = persistentChromaClient.get_collection(
                        collection, embedding_function=pyEmbedFunction
                    )
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run the Chroma Flow Studio application')
    parser.add_argument('--host', type=str, default='0.0.0.0', help='The host to bind to (default: 0.0.0.0 for all interfaces)')
    parser.add_argument('--port', type=int, default=port_flask, help=f'The port to bind to (default: {port_flask})')
    args = parser.parse_args()
    
    # Start a separate thread to open the browser (only if running locally)
    if args.host in ['127.0.0.1', 'localhost']:
        threading.Thread(target=open_browser).start()
    
    print(f"\n{'='*80}")
    print(f"Starting Chroma Flow Studio on http://{args.host}:{args.port}")
